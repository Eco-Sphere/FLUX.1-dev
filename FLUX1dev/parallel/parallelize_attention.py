...
class FluxAttnProcessor2_0_TxtNonSplit(): 
    ...
    def forward_non_overlap(
        ...
        encoder_hidden_states_full = torch.empty([self.world_size, *encoder_hidden_states_shape], dtype=encoder_hidden_states.dtype, device=encoder_hidden_states.device)
        torch.distributed.all_gather_into_tensor(encoder_hidden_states_full, encoder_hidden_states.contiguous(), group=self.group)
        encoder_hidden_states = encoder_hidden_states_full.permute(1, 0, 2, 3, 4).reshape(encoder_hidden_states_shape[0], -1, encoder_hidden_states_shape[2], encoder_hidden_states_shape[3])
