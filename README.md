...
inline void* AllocateWorkspace(uint64_t workspaceSize, at::Tensor& workspaceTensor)
{
    if (workspaceSize == 0) {
        return nullptr;
    }
...
template <
    ...
    typename WorkspaceAddrType
    >
void RunOpCommand(
    aclrtStream aclStreamObj, WorkspaceAddrType workspaceAddr, uint64_t workspaceSize,
...
{
    auto aclCall = [convertedParams, workspaceAddr, workspaceSize, aclStreamObj,
        opApiFuncAddr, releaseMemAddr, executor]() -> int {
        using OpApiFunc = int (*)(FunctionPtr<>, uint64_t, AclOpExecutor*, const aclrtStream);
        auto opApiFunc = FunctionPointerConverter<OpApiFunc, OpApiFuncAddrType>::Convert(opApiFuncAddr);
...
template<typename T = void>
void *GetOpApiFuncAddrInLib(T *handler, const char *libName, const char *apiName);
void* GetFuncFromDefaultLib(const char* apiName);
