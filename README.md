...
class FluxPipelineWrapper(FluxPipeline):
    ...
    def __call__(
        ...
        print(f"==================================>>> {txt_seq_length}\ {img_seq_length}")

        img_split_state = get_split_states("img")
        if img_split_state:
            # 等分SP
            image_rotary_emb_sp = get_rotary_emb_sp_equal(image_rotary_emb, text_ids.shape[0], self.ulysses_world_size)
        else:
            image_rotary_emb_sp = get_rotary_emb_sp_unequal(image_rotary_emb, text_ids.shape[0], self.ulysses_world_size)

        if img_split_state:
            latents = split_forward_gather(latents, self.ulysses_world_size, self.ulysses_rank, dim=1)
        else:
            split_size_list = get_split_seq_list("img")
            latents = torch.split(latents, split_size_list, dim=1)[self.ulysses_rank].contiguous()

        #TODO 不切分txt的话需要注释掉
        prompt_embeds = split_forward_gather(prompt_embeds, self.ulysses_world_size, self.ulysses_rank, dim=1)
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                if XLA_AVAILABLE:
                    xm.mark_step()
        if img_split_state:
            latents = gather_forward_split_equal(latents, self.ulysses_group, self.ulysses_world_size) 
        else:
            latents = gather_forward_split_unequal(latents, self.ulysses_group, self.ulysses_world_size) 
